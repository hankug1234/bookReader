...
if __name__ == '__main__':
    ...
    parser.add_argument("--sr", type=str, default="40k" ,choices=["40k","48k"])
    parser.add_argument("--if_f0", default=True,choices=[True,False])
    parser.add_argument("--train_dir", type=str, required=True)
    parser.add_argument("--spk_id", default=0,type=int)
    parser.add_argument("--np", default=22, type=int)
    parser.add_argument("--f0method", default="rmvpe",choices=["pm", "harvest", "crepe", "rmvpe","rmvpe_gpu"])
    parser.add_argument("--save_epoch", default=100,type=int)
    parser.add_argument("--total_epoch", default=500,type=int)
    parser.add_argument("--batch_size", default=16, type=int)
    parser.add_argument("--if_save_latest", default="no",choices=["no","yes"])
    parser.add_argument("--pretrained_G",type=str,  default="/app/assets/pretrained_v2/f0G40k.pth")
    parser.add_argument("--pretrained_D",type=str,  default="/app/assets/pretrained_v2/f0D40k.pth")
    parser.add_argument("--gpus", default="0")
    parser.add_argument("--if_cache_gpu", default="no",choices=["no","yes"])
    parser.add_argument("--if_save_every_weights", default="no",choices=["no","yes"])
    parser.add_argument("--version", default="v2", choices=["v1","v2"])
    parser.add_argument("--gpus_rmvpe", default="0-0")
        ...
        if if_f0_3:
            for _ in range(2):
                opt.append(
                    ...
                    % ("/app", sr2, "/app", fea_dim, "/app", "/app", spk_id5)
                )
        else:
            for _ in range(2):
                opt.append(
                    "%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s"
                    % ("/app", sr2, "/app", fea_dim, spk_id5)
                )
        shuffle(opt)
    ...
    for log in train_oneshot(args.exp_dir, args.sr, args.if_f0, args.train_dir, args.spk_id, args.np, args.f0method, args.save_epoch, args.total_epoch, args.batch_size, args.if_save_latest,
                            args.pretrained_G, args.pretrained_D, args.gpus, args.if_cache_gpu, args.if_save_every_weights, args.version, args.gpus_rmvpe):
        print(log)
    
    os.makedirs(f'{args.export_path}/{args.exp_dir}', exist_ok=True)
    
    src1 = f'/app/assets/weights/{args.exp_dir}.pth'
    dest1 = f'{args.export_path}/{args.exp_dir}'
    shutil.move(src1,dest1)
    
    src2 = f'/app/logs/{args.exp_dir}'
    dest2 = f'{args.export_path}/{args.exp_dir}/logs'
