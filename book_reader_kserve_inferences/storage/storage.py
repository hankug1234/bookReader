...
class Storage(object):  # pylint: disable=too-few-public-methods
    ...
    def download(uri: str, out_dir: str = None) -> str:
        ...
        elif uri.startswith(_MINIO_PREFIX):
            Storage._download_minio(uri,out_dir)
    ...
    def _update_with_storage_spec():
        ...
        print(storage_secret_json)
        if storage_secret_override_params:
            for key, value in storage_secret_override_params.items():
                storage_secret_json[key] = value
        
        if storage_secret_json.get("type", "") == "minio":
            os.environ["MINIO_ENDPOINT_URL"] = storage_secret_json.get("endpoint_url", "")
            os.environ["MINIO_ACCESS_KEY_ID"] = storage_secret_json.get("access_key_id", "")
            os.environ["MINIO_SECRET_ACCESS_KEY"] = storage_secret_json.get("secret_access_key", "")
    ...
    @staticmethod
    def _download_minio(uri, temp_dir: str):
        
        parsed = urlparse(uri, scheme='minio')
        bucket_name = parsed.netloc
        object_name = parsed.path.lstrip('/')
        
        client = Minio(os.getenv("MINIO_ENDPOINT_URL"), access_key=os.getenv("MINIO_ACCESS_KEY_ID"),secret_key=os.getenv("MINIO_SECRET_ACCESS_KEY"),secure=False)
        
        target = f"{temp_dir}/{os.path.basename(parsed.path)}"
        
        if not os.path.exists(temp_dir):
            os.makedirs(temp_dir, exist_ok=True)
        
        client.fget_object(bucket_name=bucket_name,object_name=object_name,file_path=target)
        logging.info('Downloaded object %s to %s' % (object_name, target))
